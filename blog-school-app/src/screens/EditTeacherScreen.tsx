...
export default function EditTeacherScreen({ navigation, route }: any) {
  const { user, teacher } = route.params || {};
  ...
  const loadTeacherData = async () => {
    ...
    try {
      const response = await getPerson(route.params.teacherId);
  ...
  const handleUpdateTeacher = async () => {
    ...
    const teacherId = teacher?.id || route.params?.teacherId;
    if (!teacherId) {
    ...
    try {
      ...
      await updatePerson(teacherId, updateData);
